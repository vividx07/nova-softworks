...
do -- Initialize
    ...
    local ESP_Function = function(plr)
        ...
        local function UpdateESP()
            ...
            -- Team check logic - Fixed to handle when TeamCheck is disabled
            local shouldShowESP = false
            if ESP.TeamCheck then
                -- Only show ESP for enemies (non-teammates) when TeamCheck is enabled
                shouldShowESP = plr ~= lplayer and (not isTeammate(plr.Character))
            else
                -- Show ESP for all players except local player when TeamCheck is disabled
    ...
    -- Refresh function to reinitialize ESP when settings change
    ...
    -- Force update function that can be called when enabling ESP
    getgenv().ForceUpdateESP = function()
        -- This doesn't recreate everything, just forces a state refresh
        -- The render loop will handle the rest automatically
        print("ESP Force Update - Enabled:", ESP.Enabled)
    end
