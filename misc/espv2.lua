...
do
    ...
    function Functions:CleanupAll()
        ...
        -- Disconnect main render connection
...
do -- Initialize
    ...
    local CreatePlayerESP = function(plr)
        ...
        -- Create UI elements - ALL ELEMENTS CREATED REGARDLESS OF INITIAL STATE
        ...
        -- Store player data for the main render loop
        PlayerESPData[plr] = {
            Elements = elements,
            Name = Name,
            Distance = Distance,
    ...
    -- MAIN RENDER LOOP - HANDLES ALL PLAYERS IN ONE LOOP
    local function MainRenderLoop()
        -- If ESP is globally disabled, hide everything and return
        ...
        for plr, data in pairs(PlayerESPData) do
            ...
            -- Hide elements function for this player
            ...
            if ESP.TeamCheck then
                shouldShowESP = plr ~= lplayer and (not isTeammate(plr.Character))
            else
            ...
            if ESP.Drawing.Chams.Enabled then
                data.Chams.Adornee = plr.Character
                data.Chams.Enabled = true
                data.Chams.FillColor = ESP.Drawing.Chams.FillRGB
                data.Chams.OutlineColor = ESP.Drawing.Chams.OutlineRGB
                
                if ESP.Drawing.Chams.Thermal then
                    local breathe_effect = math.atan(math.sin(tick() * 2)) * 2 / math.pi
                    data.Chams.FillTransparency = ESP.Drawing.Chams.Fill_Transparency * breathe_effect * 0.01
                    data.Chams.OutlineTransparency = ESP.Drawing.Chams.Outline_Transparency * breathe_effect * 0.01
                end
                
                data.Chams.DepthMode = ESP.Drawing.Chams.VisibleCheck and "Occluded" or "AlwaysOnTop"
            else
            ...
            if ESP.Drawing.Boxes.Corner.Enabled then
                ...
                data.BottomRightSide.Visible = true
                ...
                data.BottomRightDown.Visible = true
                ...
                data.LeftTop.Visible = false
                data.LeftSide.Visible = false
                data.BottomSide.Visible = false
                data.BottomDown.Visible = false
                data.RightTop.Visible = false
                data.RightSide.Visible = false
                data.BottomRightSide.Visible = false
                data.BottomRightDown.Visible = false
            end
            ...
            if ESP.Drawing.Boxes.Full.Enabled then
                ...
                data.Gradient1.Enabled = ESP.Drawing.Boxes.GradientFill
                data.Gradient2.Enabled = ESP.Drawing.Boxes.Gradient

                if ESP.Drawing.Boxes.Filled.Enabled then
                    data.Box.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
                    data.Box.BackgroundTransparency = ESP.Drawing.Boxes.GradientFill and ESP.Drawing.Boxes.Filled.Transparency or 1
            ...
            if ESP.Drawing.Healthbar.Enabled then
                ...
                data.Healthbar.Size = UDim2.new(0, ESP.Drawing.Healthbar.Width, 0, h * health)  
                
                data.BehindHealthbar.Visible = true
                data.BehindHealthbar.Position = UDim2.new(0, Pos.X - w / 2 - 6, 0, Pos.Y - h / 2)  
                data.BehindHealthbar.Size = UDim2.new(0, ESP.Drawing.Healthbar.Width, 0, h)
                
                -- Update gradient dynamically
                data.HealthbarGradient.Enabled = ESP.Drawing.Healthbar.Gradient
                ...
                if ESP.Drawing.Healthbar.HealthText then
                    ...
                    data.HealthText.Visible = Humanoid.Health < Humanoid.MaxHealth
                    ...
                    if ESP.Drawing.Healthbar.Lerp then
                        ...
                        data.HealthText.TextColor3 = ESP.Drawing.Healthbar.HealthTextRGB
                    end
                else
                ...
                data.BehindHealthbar.Visible = false
                data.HealthText.Visible = false
            end
            ...
            if ESP.Drawing.Names.Enabled then
                ...
                if ESP.Options.Friendcheck and lplayer:IsFriendsWith(plr.UserId) then
                    data.Name.Text = string.format('(<font color="rgb(%d, %d, %d)">F</font>) %s', ESP.Options.FriendcheckRGB.R * 255, ESP.Options.FriendcheckRGB.G * 255, ESP.Options.FriendcheckRGB.B * 255, plr.Name)
                else
            ...
            if ESP.Drawing.Distances.Enabled then
                ...
                elseif ESP.Drawing.Distances.Position == "Text" then
                    ...
                    if ESP.Drawing.Names.Enabled then
                        if ESP.Options.Friendcheck and lplayer:IsFriendsWith(plr.UserId) then
                            data.Name.Text = string.format('(<font color="rgb(%d, %d, %d)">F</font>) %s [%d]', ESP.Options.FriendcheckRGB.R * 255, ESP.Options.FriendcheckRGB.G * 255, ESP.Options.FriendcheckRGB.B * 255, plr.Name, math.floor(Dist))
                        else
            ...
            if ESP.Drawing.Weapons.Enabled then
                data.Weapon.Text = getWeaponName(plr)
                data.Weapon.Visible = true
                -- Update gradient dynamically
                data.Gradient3.Enabled = ESP.Drawing.Weapons.Gradient
            else
    ...
    -- START THE MAIN RENDER LOOP - This runs constantly regardless of ESP state
    ...
    -- Force refresh function - recreates all ESP elements
    getgenv().RefreshESP = function()
        ...
        -- Reinitialize ESP for all players
        for _, v in pairs(Players:GetPlayers()) do
            if v.Name ~= lplayer.Name and v.Parent then
                CreatePlayerESP(v)
            end      
        end
    ...
    getgenv().DebugESP = function()
        ...
        print("ESP.TeamCheck:", ESP.TeamCheck)
        print("ESP.Drawing.Chams.Enabled:", ESP.Drawing.Chams.Enabled)
        print("ESP.Drawing.Names.Enabled:", ESP.Drawing.Names.Enabled)
        print("PlayerESPData count:", 0)
        for plr, data in pairs(PlayerESPData) do
    ...
    getgenv().ESPCleanupConnection = Players.PlayerRemoving:Connect(function(plr)
        if plr == lplayer then
            getgenv().CleanupESP()
        end
    end)
    ...
    print("Use getgenv().DebugESP() to debug issues")
    print("Use getgenv().RefreshESP() to refresh if needed")
end;
